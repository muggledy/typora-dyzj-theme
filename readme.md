# typora-dyzj-theme

<p>
<a href="https://typora.io/windows/dev_release.html" alt="null"><img src="https://img.shields.io/badge/Typora-v0.9.98-green?style=flat&logo=data:image/jpeg;base64,"></a>
<a href="https://github.com/muggledy/typora-dyzj-theme/releases/latest" alt="null">
  <img src="https://img.shields.io/github/v/release/muggledy/typora-dyzj-theme"/>
  <img src="https://img.shields.io/github/downloads/muggledy/typora-dyzj-theme/total.svg"/>
</a>
</p>

> 最开始下载了[My-Typora-Themes](https://github.com/Theigrams/My-Typora-Themes)主题，之后在使用过程中不断修改（主要保留了其经典的标题样式、图片阴影样式、任务列表样式），得到目前的自用款式，主要新增了有序/无序列表样式、超链接样式、提示块样式以及一些文本样式等。相关样式来源参见致谢名单

![主题截图](./temp/dyzjposter.png)

## 使用指南

1. 依次点击Typora菜单栏的“文件”-“偏好设置”，然后点击“外观”-“打开主题文件夹”
2. 下载本主题样式，解压缩至上述“主题文件夹”（其中“dyzj-light.css”、“dyzj-dark.css”和“source/”文件是必须的，其他可以删除），重启Typora，点击Typora菜单栏的“主题”，选择“Dyzj Light”（明亮模式）或“Dyzj Dark”（暗黑模式）选项即可
3. 打开下载的“demo.md”查阅本主题所有样式，或者前往<a href="https://typora-dyzj-theme.vercel.app/">地址</a>在线预览

<details>
    <summary>修改主题配色或字体（可选操作）</summary>
    <br>
    编辑<code>dyzj-light.css</code>或<code>dyzj-dark.css</code>配置文件：
    <pre>
    --select-text-bg-color: #ffafa3; /* 文本选中背景色 */
    --bg-color: white; /* 整体背景色 */
    --sidebarbg-color: white; /* 侧边栏背景色 */
    --focus-color: #ff5d52; /* 点击某些元素（如标题）显示的颜色 */
    --codeblockbg-color: #eee; /* 代码块背景色 */
    --codeblockfont-color: #555; /* 代码块默认字体颜色 */
    --codeblockline-color: #e0f8f8; /* 代码块选中行颜色 */
    --titlebefore-color: #00c4b6; /* 各级标题点缀颜色 */
    --codeblocknum-color: rgba(128, 128, 255, 0.8); /* 代码块行编号颜色 */
    --blockquoteicon-color: #5fa7e4; /* 提示块图标色 */
    --blockquotebg-color: #f4fcff; /* 提示块背景色 */
    --blockquotedangericon-color: #db524f; /* 危险型提示块图标色 */
    --blockquotedangerbg-color: #fcf1f2; /* 危险型提示块背景色 */
    --blockquotesuccessicon-color: #5cb85c; /* 成功型提示块图标色 */
    --blockquotesuccessbg-color: #eef7f0; /* 成功型提示块背景色 */
    --blockquotewarnicon-color: #f0ad4e; /* 警告型提示块图标色 */
    --blockquotewarnbg-color: #fef8ea; /* 警告型提示块背景色 */
    --ulol-color: #81accf; /* 有序无序列表前缀颜色 */
    --olfore-color: #fff; /* 有序列表字体颜色 */
    --tableevenrow-color: #eee; /* 表格偶数行颜色 */
    --tablefocusrow-color: #e0f8f8; /* 表格鼠标悬停行颜色 */
    --tableborder-color: #c5c5c5; /* 表格边框色 */
    --tablethbg-color: #f4fcff; /* 表格表头背景色 */
    --scrollbar-color: #9e9e9e; /* 滚动条颜色 */
    --toc-color: #00a0a0; /* 目录颜色 */
    --atext-color: black; /* 超链接文本颜色 */
    --ahover-color: #ffffffe6; /* 超链接悬停文本颜色 */
    --abottom-color: #81d8cf; /* 超链接下划线颜色 */
    --code-color: #ff3502; /* 行内代码前景色 */
    --codebg-color: #f8f5ec; /* 行内代码背景色 */
    --footnote-color: #e96900; /* 脚注上标前景色 */
    --footnotebg-color: #f8f8f8; /* 脚注上标背景色 */
    --highlight-color: red; /* 高亮文本前景色 */
    --highlightbg-color: yellow; /* 高亮文本背景色 */
    --split-color: #00c4b6; /* 分割线颜色 */
    --taskborder-color: red; /* 任务列表边框色 */
    --taskfocus-color: #00c4b6; /* 任务列表选中填充色 */
    --spannote-color: #e91e64; /* 默认小标签背景色 */
    --emptext-color: #fe5f58; /* 几款强调型文本的颜色 */
    --hidetextcover-color: #a1a1a1; /* 隐藏文本遮罩颜色 */
    --hidetext-color: #444; /* 隐藏文本颜色 */
    --spanbtn-color: #44d7b6; /* 文本按钮颜色 */
    --detailstxt-color: rgba(68,68,68,.7); /* 折叠框标题文字颜色 */
    --detailsbg-color: #ebf9ed; /* 折叠框背景色 */
    --detailsborder-color: #00c4b6; /* 折叠框边框色 */
    --body-color: #000000; /* 全局文本颜色 */
    --para-color: #000000; /* 段落文本颜色 */
    --commonstr-color: #232629; /* 常规字符串颜色 */
    --commonstrbg-color: #e3e6e8; /* 常规字符串背景色 */
    --timebodybg-color: #efeded; /* 时间轴主体部分背景色 */
    --sidelist-selected-color: #eeeeee; /* 侧边栏条目选中背景色 */
    --yamlbg-color: #f7f7f7; /* 头部YAML背景色 */
    --yaml-color: #585858; /* 头部YAML字体颜色 */
    --body-font: 'Source Sans Pro'; /* 全局字体 */
    --code-font: 'JetBrains Mono'; /* 代码块以及行内代码字体 */
    --bgimg-url: url(./bg-light.gif); /* 右下角图片 */
    </pre>
</details>

<font>致谢</font>：[zj主题作者](https://github.com/Theigrams/My-Typora-Themes)、[butterfly主题](https://github.com/jerryc127/hexo-theme-butterfly)、[volantis主题](https://volantis.js.org/)
